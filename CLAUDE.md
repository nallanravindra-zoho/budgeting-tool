...
20. **Employee headcount charts: bar → pie with % share (2026-08-29)** — "Headcount by Department" and "Headcount by Location" (renamed "Headcount by Country" — it's keyed by each employee's `country` field, see `computeEmployeeDashboardStats`'s `byLocation` in `employeeData.js`; the panel title was just imprecise before) switched from horizontal bar charts to pie charts, each slice labeled with its % of total headcount and a value+% tooltip on hover. New shared `EmployeePieChart` component (reused by both panels) rather than duplicating the same recharts config twice.
