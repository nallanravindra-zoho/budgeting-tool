...
13. **Overview page redesign (2026-08-28)** — one-stop COO landing page, per `overview-vendors-redesign-spec.md` (Vendors-page half of that spec not yet built): Management Snapshot bar (deterministic headline + facts + clickable topic chips, no cash-flow content), 4 matched-baseline KPI tiles (YTD Actual vs YTD Plan, FY Forecast vs FY Budget), company-wide FY System Forecast now computed at the App level (`enrichedVendors`, reusing `computeFySystemForecast` per vendor) and added as a third series to the Revenue/GP charts — dashed, distinct color, picking up exactly where the solid Actual line ends; GP chart dropped its second axis in favor of a text GP% callout. Key Drivers panel (top revenue contributors, top margin detractors, top regions) revives the `movers` computation that existed but was never rendered before. Risks & Opportunities panel: GP softness, vendor concentration, one chart-anomaly observation — deliberately no pipeline/renewal item, since no real data source for that exists anywhere in this schema. Also fixed a real bug found along the way: `getActualCutoffMonthIndex` (`firestoreData.js`) was returning the *current, still-open* month instead of the last *closed* one — inflated "actuals" by one partial month everywhere it's used (Overview, Vendors, Regions, FY forecast baseline), not just here. TopBar gained a persisted "Actuals through [month]" line and a "last synced" indicator that now survives a page reload (reads back the `lastSyncedAt` `syncCipr` already writes per vendor, previously never surfaced client-side).
